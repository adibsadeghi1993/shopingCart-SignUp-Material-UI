...
const Cart = ({cart,decreaseHandler,increaseHandler}) => {
    // console.log(cart)
    ...
    useEffect(() => {
       const price=  cart.reduce(
        (previous, current)=>previous+current.price.raw*current, 
        0);
          setTotalPrice(price)
     ...
     <Grid container spacing={3} >
         {cart.map((item)=>(
             <Grid item xs={12} sm={4} key={item.id}>
                 <CartItem item={item} increaseHandler={increaseHandler} decreaseHandler={decreaseHandler}     />
